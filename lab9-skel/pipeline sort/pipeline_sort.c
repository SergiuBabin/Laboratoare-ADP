...
int main(int argc, char * argv[]) {
	...
	if (rank == 0) { // This code is run by a single process
		...
		for (int i = 0; i < nProcesses - 1; i++) {
			MPI_Recv(&v[i], 1, MPI_INT, i + 1, 0, MPI_COMM_WORLD, &status);
		}
		...
		int array_value;
		MPI_Status status;
		MPI_Recv(&array_value, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);

		if (rank == 1) {
			int arr[nProcesses - 1];
			MPI_Recv(arr, nProcesses - 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
			for (int i = 1; i < nProcesses - 1; i++) {
				array_value = arr[i];
			}

			MPI_Send(&array_value, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
		}
		else {
			...
			for (int i = 1; i < nProcesses - rank; i++) {
				...
				if (array_value == -1) {
					array_value = recv_value;
				} else if (array_value <= recv_value) {
					MPI_Send(&recv_value, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
				} else {
					MPI_Send(&array_value, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
					array_value = recv_value;
				}
			}
